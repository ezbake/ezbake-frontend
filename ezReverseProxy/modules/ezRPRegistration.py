...
def getUpstreamHostAndPort(registration):
  ...
  if len(hap) != 2:
      raise EzReverseProxy.RegistrationInvalidException("registration.UpstreamHostAndPort: %s is not of the form hostname:port" % (registration))
  ...
  try:
      ...
      raise EzReverseProxy.RegistrationInvalidException("registration.UpstreamHostAndPort: %s port does not seem to be an integer in the range [1,65535]" % (registration))
  return hap[0],port
